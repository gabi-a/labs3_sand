...
parser.add_argument("-m","--minlength",help="max sidelength of grid",type=int,default=5)
...
sizes = np.random.choice(np.arange(min_length, max_length), (N,2))
...
dirname = f"{N}_{n_drops}_{min_length}_{max_length}"
...
for i in range(N):
    ...
    try:
        sandpile.run()
[i] = find_linear_region(sandpile.avalanche_, sizes[i,0]*sizes[i,1], "size")
